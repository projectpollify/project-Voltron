...
**STATUS: DRAFT v4.0 — the CONTINUITY SPECIFICATION. Concept exploration, awaiting owner ratification.**

> **One of three documents.** This is the *product*: what makes a
> composition the same continuing entity. `DRIFT_EXTENSION.md` is an
> extension addressing behavioural fidelity. `VOLTRON_STATIC.md` is a
> research protocol, not a product. Read this one first; the other two
> depend on it.
...
## 12.5 Behavioural drift — see the extension

Unchanged commitments do not guarantee unchanged interpretation. That
problem, and the machinery answering it — calibration probes, the
conscience organ, dispute, and the `restore` change type — live in a
separate document: **`DRIFT_EXTENSION.md`**.

It is filed separately on purpose. This specification answers *what
makes a composition the same continuing entity*. The extension answers
a different question — *does the entity still mean what it said* — and
conflating them made both harder to judge. `restore` remains defined
here (§4, §11) because it is a change type the verifier must know;
everything else about drift is the extension's.
...
**v4.0 (2026-08-03).** Split by kind. The drift compass moved to
`DRIFT_EXTENSION.md` and the frozen-baseline protocol to
`VOLTRON_STATIC.md`, leaving this document to answer one question:
what makes a composition the same continuing entity. Nothing was
deleted and no rule changed; `restore` stays here because the verifier
must know it.
